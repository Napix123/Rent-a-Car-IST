...
routerAutmobil.put("/AutomobilIzmena/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "Greska prilikom updejtivanja automobili:" });
  } finally {
    if (client) {
      await client.close();
    }
...
routerAutmobil.get("/AutomobilDatumPretraga", async (req, res) => {
  ...
  console.error(
    "########## Od " + Od + "   DO" + Do + " Pretraga " + strPretrage
  );
  try {
    client = await Konekcija();
    const db = client.db(dbName);
    ...
    dokumenti = dokumenti.filter((el) =>
      el.model.toUpperCase().includes(strPretrage.toUpperCase())
    );
    res.json(dokumenti);
  } catch (err) {
...
routerAutmobil.delete("/Automobil/:id", async (req, res) => {
  ...
  try {
    ...
    await db
      .collection("Automobili_History")
      .insertOne({ Auto, datumBrisanja: new Date() });
...
routerAutmobil.get("/AutomobilSortiraj/:tip", async (req, res) => {
  ...
  try {
    switch (tip) {
      case "modelOpadajuce":
        dokumenti.sort((a, b) => b.model.localeCompare(a.model));
        break;
      case "modelRastuce":
        dokumenti.sort((a, b) => a.model.localeCompare(b.model));
        break;
      case "cenaRastuce":
        dokumenti.sort((a, b) => a.cena - b.cena);
        break;
      case "cenaOpadajuce":
        dokumenti.sort((a, b) => b.cena - a.cena);
        break;
      default:
