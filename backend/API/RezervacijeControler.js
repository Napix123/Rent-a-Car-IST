    ...
    await db.collection("Rezervacije").updateOne(
      { _id: new ObjectId(req.body._id) },
      {
        $set: {
          ...
          cenaPoDanu: req.body.cenaPoDanu,
          ukupnaCena:
            Razlika(req.body.datumOd, Date.now()) * req.body.cenaPoDanu,
          datumDo: Date.now,
        },
      }
    );
...
routerRezervacija.delete("/Rezervacije/:id", async (req, res) => {
  ...
  try {
    ...
    await db
      .collection("Rezervacije_History")
      .insertOne({ Rezervacija, datumBrisanja: new Date() });
...
routerRezervacija.get("/RezervacijeAutomobil", async (req, res) => {
  ...
  console.log(
    "AUTOOOOOO je " + AutoId + "   OD " + datumOd + " datumDo" + datumDo
  );
  try {
    client = await Konekcija();
    const db = client.db(dbName);
    let dokumenti = await db
      .collection("Rezervacije")
      .find({
        AutoId: AutoId,
      })
    ...
    dokumenti = dokumenti.filter((el) => {
      return (
        IzmedjuDvaDAtuma(datumOd, datumDo, el.datumOd) &&
        IzmedjuDvaDAtuma(datumOd, datumDo, el.datumDo)
      );
    });
